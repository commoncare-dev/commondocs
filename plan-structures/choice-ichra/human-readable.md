...
| Product question | CHOICE approach |
|---|---|
| Who selects the insurance plan? | The participant, from eligible individual-market options |
| How are plans compared? | CommonCare ranks total economic performance using realistic multiyear simulations |
| Can personal needs override the ranking? | Yes; the ranking supplies data, not a command |
| How much goes into the ICHRA? | Only the amount needed to accomplish the employer’s ICHRA objective—which may be de minimis when affordability treatment is not required |
| Where does the rest of the employer allowance go? | A flex or health-flex structure that preserves broader qualified uses |
| How are unreimbursed premiums paid pre-tax? | Through Section 125 salary reduction for eligible off-Exchange coverage |
| Can Marketplace coverage be used? | An ICHRA can integrate with on- or off-Exchange individual coverage, but Section 125 cannot pay Exchange premiums |
| What happens to premium-tax-credit eligibility? | It depends on ICHRA affordability, acceptance, and a valid opt-out |
...
| CommonCare supplies | The participant decides |
|---|---|
| Scrupulous cost analysis | How much they value a particular doctor or hospital |
| Expected and worst-case economic outcomes | Whether a narrow network feels workable |
| Break-even points | How much uncertainty they are comfortable accepting |
| Prescription and provider compatibility | Whether convenience justifies a measurable additional cost |
| Identification of dominated plans | Whether a personal preference is worth overriding the ranking |
...
|---|---|
| ICHRA amount | Establish the employer-sponsored individual-coverage reimbursement and, when intended, support formal ICHRA affordability |
| Health-flex credit | Restrict employer dollars to permitted medical care and qualified health options while preserving broader healthcare uses |
| General flex credit | Preserve additional participant choice, including any permitted cash or taxable options under the employer’s design |
...
|---|---:|---|
| ICHRA | \$25 | Employer reimbursement conditioned on qualifying individual coverage |
| Flex allowance | \$575 | Available under the applicable cafeteria-plan terms, including any cashable election |
| Premium remaining after ICHRA | \$450 | Paid through the Section 125 arrangement because the participant is covered by the ICHRA and the policy is off-Exchange |
...
| Intended outcome | Question to scrutinize |
|---|---|
| Ensure access to major medical coverage | Does the incentive actually increase suitable enrollment, or merely punish employees with other coverage? |
| Improve employee health | Is buying additional insurance the intervention most likely to produce that result? |
| Protect employees from large bills | Does the selected coverage meaningfully reduce maximum exposure and provider risk? |
| Satisfy ACA affordability | Is the amount placed in the vehicle that receives credit under the applicable affordability rule? |
| Deliver tax-efficient compensation | Does the restriction preserve enough utility to make the tax advantage worthwhile? |
| Meet employee expectations | Are those expectations informed by total economics or by deductible-focused insurance marketing? |
...
| Requirement | CHOICE implementation |
|---|---|
| Individual coverage | The participant must substantiate qualifying individual coverage or Medicare for each covered period |
| Employer funding | The ICHRA is employer-funded; employee salary reduction cannot fund the ICHRA itself |
| Same-class plan choice | An employer generally cannot offer both a traditional group plan and an ICHRA to the same employee class |
| Class design | Use permitted employee classes and applicable minimum-class-size rules |
| Same terms | Offer the ICHRA on the same terms within a class, subject to permitted age and family-size variation |
| Written documents | Maintain the governing plan document and required participant materials |
| Notice | Provide the required ICHRA notice generally 90 days before the plan year or when a newly eligible employee can timely act |
| Opt-out | Permit the required annual and termination opt-outs |
| Claims | Substantiate coverage and reimbursable expenses before payment |
| Reporting | Complete applicable Forms 1094-C and 1095-C and other required reporting |
...
| ICHRA offer | Participant action | Potential PTC result |
|---|---|---|
| Affordable | Accepts or opts out | No premium tax credit |
| Unaffordable | Accepts ICHRA | No premium tax credit |
| Unaffordable | Opts out and otherwise qualifies | May receive premium tax credit |
